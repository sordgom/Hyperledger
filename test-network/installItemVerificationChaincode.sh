...
CHAINCODE_NAME="ItemVerification"
CHAINCODE_VERSION="1"
CHAINCODE_PATH="../chaincode/ItemVerification/build/install/ItemVerification"
CHAINCODE_LANG="java"
CHAINCODE_LABEL="certification_3"
...
chaincodeAddCert() {
    ...
    --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1\
    --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2\
    -c '{"Args":["createCert", "CERT1", "Charika1","drb mila","mghrib","charika2","drb milaaat","mghrib2","yawm istiqlal","daba","bato","mrsa","1","1","1","1","1","1","1","1","1"]}'
    echo "===================== Successfully Added New Cert===================== "
}

chaincodeQueryCertById() {
    echo "===================== Started Query Cert By Id Chaincode Function===================== "
    setEnvVarsForPeer0Org1
    peer chaincode invoke -o $ORDERER_ADDRESS\
    --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED\
    --cafile $ORDERER_CA -C ${CHANNEL_NAME} --name ${CHAINCODE_NAME}\
    --peerAddresses localhost:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG1\
    --peerAddresses localhost:9051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE_ORG2\
    -c '{"Args":["queryCert", "CERT1"]}'
    echo "===================== Successfully Invoked Cert  By Id Chaincode Function===================== "
}
